...
from .models import Superhero, Animal, Car, Employee, Stock, Message, Credit_Card, Blog, Friend, Graffiti, Food, Contact, Athlete, Task, Ajax_Task
...
def superheroJSON(request):
    
    superheroes = Superhero.objects.all()
    superheroes_json = serializers.serialize('json', superheroes)
    
    return HttpResponse(superheroes_json, content_type='application/json')

def animalJSON(request):
...
def stockJSON(request):
    ...
    return HttpResponse(stocks_json, content_type='application/json')

def messageJSON(request):
...
def credit_cardJSON(request):
    
    credit_cards = Credit_Card.objects.all()
    credit_cards_json = serializers.serialize('json', credit_cards)
    
    return HttpResponse(credit_cards_json, content_type='application/json')
...
def graffitiJSON(request):
    
    graffitis = Graffiti.objects.all()
    graffitis_json = serializers.serialize('json', graffitis)
    
    return HttpResponse(graffitis_json, content_type='application/json')
    
def foodJSON(request):
...
def athleteJSON(request):
    
    athletes = Athlete.objects.all()
    athletes_json = serializers.serialize('json', athletes)
    
    return HttpResponse(athletes_json, content_type='application/json')
...
def ajax_taskJSON(request):
    
    ajax_tasks = Ajax_Task.objects.all()
    ajax_tasks_json = serializers.serialize('json', ajax_tasks)
    
    return HttpResponse(ajax_tasks_json, content_type='application/json')